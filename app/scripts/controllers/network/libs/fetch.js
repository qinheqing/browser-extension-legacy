...
export function createFetchMiddleware ({ rpcUrl, originHttpHeaderKey, fallbackUrls }) {
  return createAsyncMiddleware(async (req, res, next) => {
    ...
    for (let attempt = 0; attempt < maxAttempts; attempt++) {
      try {
        let rUrl = getFallbackUrl(rpcUrl, attempt, fallbackUrls);
        const { fetchUrl, fetchParams } = createFetchConfigFromReq({ req, rpcUrl: rUrl, originHttpHeaderKey })
