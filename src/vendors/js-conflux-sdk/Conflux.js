...
class Conflux {
  ...
  constructor({
    ...
    this.sendRawTransaction = this._decoratePendingTransaction(
      this.sendRawTransaction,
    );

    this.sendTransaction = this._decoratePendingTransaction(
  ...
  _formatAddress(address) {
    if (!this.networkId) {
      console.warn(
        'Conflux address: networkId is not set properly, please set it',
      );
    }
    return this.useHexAddressInParameter
      ? format.hexAddress(address)
      : format.address(address, this.networkId);
  }

  _formatCallTx(options) {
    return format.callTxAdvance(
      this.networkId,
      this.useHexAddressInParameter,
    )(options);
  }

  _formatGetLogs(options) {
    return format.getLogsAdvance(
      this.networkId,
      this.useHexAddressInParameter,
    )(options);
  }
   ...
   */
  async getBlockByHashWithPivotAssumption(
    blockHash,
    pivotBlockHash,
    epochNumber,
  ) {
  ...
  async _signTransaction(options) {
    ...
    if (options.gasPrice === undefined) {
      if (this.defaultGasPrice === undefined) {
        ...
        options.gasPrice =
          Number(gasPrice) === 0 ? CONST.MIN_GAS_PRICE : gasPrice;
      } else {
    ...
    if (options.gas === undefined || options.storageLimit === undefined) {
      ...
      const isContract =
        decodeCfxAddress(account.address).type === ADDRESS_TYPES.CONTRACT;
      if (options.data || isContract) {
        const { gasUsed, storageCollateralized } =
          await this.estimateGasAndCollateral(options);
        gas = format.big(gasUsed).times(this.defaultGasRatio).toFixed(0);
        storageLimit = format
          .big(storageCollateralized)
          .times(this.defaultStorageRatio)
          .toFixed(0);
      } else {
  ...
  async getLogs(options) {
    if (
      options.blockHashes !== undefined &&
      (options.fromEpoch !== undefined || options.toEpoch !== undefined)
    ) {
      throw new Error(
        'OverrideError, do not use `blockHashes` with `fromEpoch` or `toEpoch`, cause only `blockHashes` will take effect',
      );
    }

    const result = await this.provider.call(
      'cfx_getLogs',
      this._formatGetLogs(options),
    );
  ...
  async subscribeLogs({ address, topics } = {}) {
    const id = await this.subscribe(
      'logs',
      this._formatGetLogs({ address, topics }),
    );
